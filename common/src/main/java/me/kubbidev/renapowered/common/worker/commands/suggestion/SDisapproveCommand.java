...
@DiscordCommand(name = "suggestion_disapprove", subCommand = false)
public class SDisapproveCommand implements InteractionCommand {
    ...
    public void onPerform(CommandContext context) {
        ...
        channel.retrieveMessageById(messageId).queue(message -> {
            ...
            if (embeds.size() == 1) {
                ...
                builder.setColor(0xe63030);

                Locale locale = context.getDiscordLocale().toLocale();
                builder.setTitle(ComponentSerializer.serialize(Message.SUGGESTION_DISAPPROVE_TITLE.build(), locale));
                builder.setFooter(ComponentSerializer.serialize(Message.SUGGESTION_DISAPPROVE_FOOTER.build(), locale));
            ...
            context.reply(context.deferReply(true), Message.SUGGESTION_DISAPPROVE_RESPONSE.build());
    ...
    public Supplier<SlashCommandData> getSlashCommand() {
        return () -> Commands.slash("suggestion_disapprove", "Mark the suggestion provided as disapproved.")
